...
def process_detection(request, imx500, intrinsics, data_table, url_table, s3_client, labels, is_button_triggered=False, previous_label=None):
...
def main():
    ...
    try:
        while True:
            request = picam2.capture_request() # This is the missing line
            
            with button_press_lock:
                if button_pressed_flag[0]:
                    print("Processing button-triggered image")
                    previous_label = process_detection(request, imx500, intrinsics, data_table, url_table, s3_client, labels, is_button_triggered=True, previous_label=previous_label)
                    button_pressed_flag[0] = False
                else:
                    print("Processing regular cycle image")
                    previous_label = process_detection(request, imx500, intrinsics, data_table, url_table, s3_client, labels, previous_label=previous_label)
